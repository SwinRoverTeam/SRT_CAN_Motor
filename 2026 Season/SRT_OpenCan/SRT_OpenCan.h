...
public:
    ...
    //=========== Homing attempts =========
    bool home_motor(int home_offset); // run homing sequence, does not set homed pos = 0
    bool set_motor_pos_zero(); // set current pos = 0 without moving, use after homing or manual positioning
};
